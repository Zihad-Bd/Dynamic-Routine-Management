        ...
        for (int j = 0; j < numOfSemister; ++j)
        {
            for (int line = 1; line <= 3; ++line)
            {
                ...
                for (int slot = 0; slot <= 7; ++slot)
                {
                    ...
                    int totalHorizontalSpace = numberOfTimeSlot[code] * 9 + numberOfTimeSlot[code] - 1;
                    int remainingSpace;
                    string needToPrint;
                    if (line == 1)
                    {
...
bool checkSameCourseSameTeacherInADay(int id, int year, int day, string code, string teacherAbbreviation)
{
    int Count = 0;
    for (int i = 0; i < 8; ++i)
    {
        string presentCode = routine[id][day][year][i].courseCode;
        if (presentCode != "" && presentCode == code && routine[id][day][year][i].teachersAbbreviation == teacherAbbreviation)
        {
            ++Count;
...
void Insert(int id, int day, int year, int startTime, int slotNeed, string teacherAbbreviation, string code)
{
...
vector<vector<pair<int, int>>> findEmptySlots(int id)
{
    ...
    for (int i = 0; i < numOfWorkDay; ++i)
    {
        for (int j = 0; j < numOfSemister; ++j)
        {
            for (int k = 0; k < 8; ++k)
            {
                if (routine[id][i][j][k].courseCode == "" && make_pair(i, k) != gapPeriod && k != 4)
                {
                    emptySlots[j].eb(i, k);
                }
            }
